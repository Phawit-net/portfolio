...
const getMovingDirection = (
  ...
  // console.log("STEP 2 : เรียกใช้ getMovingDirection");

  // Pivot เป็นแกนมุม แกนy ตอนแรกมีค่า 0 ถ้าหมุนไปทางขวาจะติดลบไปเรื่อยๆ
  // left ค่าก็จะเป็น 3.14... / 2 = ประมาณ 1.5
...
const Ecctrl = forwardRef<RapierRigidBody, EcctrlProps>(
  (
    ...
    const action1Animation = !animated
      ? null
      : useGame((state) => state.action1);
    const action2Animation = !animated
      ? null
      : useGame((state) => state.action2);
    const action3Animation = !animated
      ? null
      : useGame((state) => state.action3);
    const action4Animation = !animated
      ? null
      : useGame((state) => state.action4);
    const action5Animation = !animated
      ? null
      : useGame((state) => state.action5);
    ...
    const getCurrentPosition = () => {
      // const allPos = [
      //   new THREE.Vector3(14.44, 0.91, 8.795),
      //   new THREE.Vector3(12.36, 1.7, 6.5),
      // ];
      // const distance = allPos.map((target) => {
      //   return currentPos.distanceTo(target);
      // });
      // console.log(distance[0], distance[1]);
      const targetPos = new THREE.Vector3(8, 1, 5);
      const annotationHTML = document.getElementsByClassName(
        "annotation"
      ) as HTMLCollectionOf<HTMLElement>;

      if (currentPos.distanceTo(targetPos) < 2) {
        if (annotationHTML !== undefined) {
          annotationHTML[0]?.setAttribute(
            "style",
            "background:red;pointer-events:auto"
          );
        }
      } else {
        if (annotationHTML !== undefined) {
          annotationHTML[0]?.setAttribute(
            "style",
            "background:indianred;pointer-events:none;display:none"
          );
        }
      }
    ...
    const moveCharacter = (
      ...
      // props movingObjectVelocity (x:0, y:0, z:0)
      // console.log("STEP 4 : เข้า moveCharacter Function");
      // movingDirection เป็น ค่าแกน x, z ถ้ากล้องอยู่ default แกน y =0 เวลาไปทางซ้าย (x = 1, y = .., z= 0)
      // ถ้าไปทางขวา (x = -1, y = .., z = 0) ไปตรง (x = 0, y = .., z = 1) ถอยหลัง (x = 0, y = .., z = -1)
      // ซึ่งเมื่อเราหมุนแกนกล้อง y ให้เอียงๆ (ค่าจะไม่ใช้ใน 1 ถึง -1) มันจะวิ่ง2 แกน
      ...
      // ค่าเป็น (x:0, y:0, z:0) ไม่รู้ใช้ทำอะไร
      ...
      // ค่า angle จะเปลี่ยนไปตลอดเวลาเราเดินบน Object ที่เคลื่อนที่ ถ้า พื้นจะเป็นค่าเดิม

      // console.log("STEP 6 : Setup rejection velocity", currentVel);
      /**
       * Setup rejection velocity, (currently only work on ground)
       */
      // dot product คือการหาผลลัพท์ของเวคเตอร์ในส่วนที่เป็นขนาดของเวคเตอร์ เท่านั้น
      const wantToMoveMeg = currentVel.dot(movingDirection);
      // set ความเร็วในการเคลื่อนที่
      wantToMoveVel.set(
        movingDirection.x * wantToMoveMeg,
        0,
        movingDirection.z * wantToMoveMeg
      );
      // ค่า wantToMoveVel เป็น vector
      rejectVel.copy(currentVel).sub(wantToMoveVel);

      // console.log("STEP 7 : ส่วนนี้ถ้าปิด character จะเคลื่อนที่ไม่ได้");
      /**
    ...
    if (isInsideKeyboardControls) {
      useEffect(() => {
        ...
        const unSubscribeAction5 = subscribeKeys(
          (state) => state.action5,
          (value) => {
            if (value) {
              animated && action5Animation();
            }
          }
        );
    ...
    useFrame((state, delta) => {
      // console.log("STEP 1 : useFrame");
      // Character current position
      // copy ตำแหน่งจาก Ref ใส่ใน currentPos
      if (characterRef.current) {
       ...
       */
      // ตั้งค่าให้แสงตามตัวละครไปเลื่อยๆ
      ...
      // /**
      //  * Getting all joystick control values -> อาจจะไม่ได้ใช้ joy
      //  */
       ...
       */
      // จะได้ค่า True/False เวลากด WASD
      const { forward, backward, leftward, rightward, jump, run } =
        isInsideKeyboardControls ? getKeys() : presetKeys;

      // Getting moving directions (IIFE) Immediately Invokable Function Expression
      // การประกาศฟังก์ชั่นที่ไม่ได้ขึ้นต้นด้วยคำว่า “function” และทำงานทันทีที่ถูกประกาศ จะเรียกใช้ครั้งเดียวแล้วคืน memory ทันที
      // modelEuler ค่าเริ่มต้น = 0 เวลากดเคลื่อนที่ถึงเปลี่ยนค่า
      // movingDirection Vect3 x0 y0 z0
      ...
      // pivot มาจาก useFollowCam เป็น กล้อง ที่ตาม character
      // จากข้างบน functionIIFE นี้ return ค่าตัวแลขแกน Y แล้วไปใส่ใน modelEuler.y
      // console.log(
      //   "STEP 3 : return modelEuler.y มีค่าเป็นตัวเลขตาม rotate.y ของ pivot(กล้อง)",
      //   "modelEuler.y",
      //   modelEuler.y
      // );

      // Move character to the moving direction
      // น่าจะเคลื่อนตัวละครไปตาม direction
      // delta น่าจะเป็นค่าเวลาที่เพิ่มไปเรื่อยๆของ useFrame
      // run เป็น boolean บอกว่าวิ่งไหม true/false เวลากด shift
      // slopeAngle ค่าเอียงเวลาขึ้นเนิน
      // movingObjectVelocity เป็น Vect3 ค่า 0,0,0
      ...
      /**
       *  Camera movement ตรงส่วนนี้ทำให้กล้องเคลื่อนที่ไปตามตัวละคร
       */
      pivotPosition.set(
