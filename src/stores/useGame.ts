...
export const useGame = /* @__PURE__ */ create(
  /* @__PURE__ */ subscribeWithSelector<State>((set, get) => {
    return {
       ...
       * Point to move point
       */
      moveToPoint: null as unknown as THREE.Vector3,
      ...
      idle: () => {
        set((state) => {
          if (state.curAnimation === state.animationSet.jumpIdle) {
            ...
            state.curAnimation !== state.animationSet.action5
          ) {
            return { curAnimation: state.animationSet.idle };
          }
          return {};
      ...
      walk: () => {
        set((state) => {
          if (state.curAnimation !== state.animationSet.action5) {
...
// เป็น Store (คล้าย Redux แน่ๆ ต้องมาทำความเข้าใจอีกที)
